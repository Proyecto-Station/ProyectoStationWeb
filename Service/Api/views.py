...
def students_list(request):
  if request.method == 'GET':
    ...
    serializer = UsernameSerializer(data, context={'request': request}, many=True)
...
def students_detail(request, pk):
  try:
    student = Username.objects.get(pk=pk)
  except Username.DoesNotExist:
    return Response(status=status.HTTP_404_NOT_FOUND)

  if request.method == 'PUT':
    serializer = UsernameSerializer(student, data=request.data,context={'request': request})
